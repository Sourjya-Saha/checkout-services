    ...
    <main className="max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Recorded Incidents</p>
          ...
          <span className="text-[11px] text-slate-500 font-medium mt-1 inline-block">
            {incidents.length === 0 ? "No active incidents" : `${resolvedCount} resolved by SentinelOps`}
          </span>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Remediation Strategy</p>
          <p className="text-2xl font-black text-slate-900 mt-1">Autonomous</p>
          <span className="text-[11px] text-blue-600 font-medium mt-1 inline-block">Sandboxed Forward-Fix / Rollback</span>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900">Incident Audit Log</h2>
          <span className="text-xs text-slate-500">Showing {incidents.length} recorded incident(s)</span>
        </div>

        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400 text-sm">
            Loading incidents from Supabase database...
          </div>
        ) : incidents.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
            <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold text-xl mx-auto">
              ✓
            </div>
            <h3 className="text-base font-bold text-slate-900">No Incidents Recorded Yet</h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              SentinelOps is connected to Supabase. When an incident is investigated and remediated by the agent, the structured record will be written to the <code>incidents</code> table and appear here in real-time.
            </p>
          </div>
        ) : (
          incidents.map((inc) => (
